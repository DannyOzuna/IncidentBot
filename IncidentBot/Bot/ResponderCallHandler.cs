        ...
        protected async override void CallOnUpdated(ICall sender, ResourceEventArgs<Call> args)
        {
            ...
            if (sender.Resource.State == CallState.Established)
            {
                ...
                if (sender.Resource.ToneInfo?.Tone != null)
                {
                    ...
                    {
                        case Tone.Tone1:
                            var callResponse = await Bot.RecordResponse(sender);
                            GraphLogger.Info($"Token: {callResponse.RecordingAccessToken} Location: {callResponse.RecordingLocation}");

                            HttpClient httpClient = new HttpClient();
                            httpClient.DefaultRequestHeaders.Add("Authorization", callResponse.RecordingAccessToken);

                            var getRecord = await httpClient.GetAsync(callResponse.RecordingLocation);

                            if (getRecord.StatusCode == System.Net.HttpStatusCode.OK) 
                            {
                                httpClient.DefaultRequestHeaders.Clear();
                                var url = "";
                                var apiReponse = await httpClient.GetAsync(url);
                            }
